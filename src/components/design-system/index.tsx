...
const DesignSystem = {
  ...
  YoutubeEmbed: function ({ videoId = "" }: { videoId?: string }) {
    const theme = useTheme();
    return (
      <YouTube
        videoId={videoId}
    ...
    return (
      <StyledIngredient>
        ...
        <div className="volume-area">
          {fillUp ? (
            <p className={classNames({ "fill-up": fillUp })}>Fill Up</p>
          ) : (
            <p>{volume}</p>
          )}
        </div>
  ...
  StarRating: function ({ grade = 0 }: { grade?: number }) {
    const theme = useTheme();
    const id = useId();

    const printStars = (grade: number) => {
      const starCount = 5;
      const stars = [];
      for (let i = 0; i < 5; i++) {
        stars.push(
          <Fragment key={`${id}-star-${i}`}>
            <label
              className={classNames(`rating-${starCount - i}`, {
                checked: grade >= starCount - i,
              })}
            ></label>
          </Fragment>
        );
